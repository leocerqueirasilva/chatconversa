...
class WhatsAppClient {
  ...
  async __createClient() {
    this.isDestroyed = false;
    try {
      console.log(`Initializing client for user ${this.userId}...`);
      this.client = new Client({
        puppeteer: {
          headless: true,
          args: [
...
const clients = new Map(); // Inicializa a variável clients
...
app.get('/auth', async (req, res) => {
  try {
    ...
    console.log(`auth called for user ${userId}`);
    let { qrCode, isLoggedIn } = client.getAuth();

    let counter = 0;
    while (!qrCode && !isLoggedIn) {
      console.log('Waiting for QR code...' + counter++);
      await new Promise((resolve) => setTimeout(resolve, 500));
      let response = client.getAuth();
...
app.post('/send-message', async (req, res) => {
  try {
    ...
    let number = numbers[0]?.charAt(0) === '+' ? numbers[0].substring(1) : numbers[0];
    number = number + '@c.us';
    const clientInstance = await client.getClient();

    if (text) {
      await clientInstance.sendMessage(number, text);
    }
    if (mediaLink) {
      const media = await axios.get(mediaLink, {
        responseType: 'arraybuffer',
      ...
      const mediaFilename = mediaLink.split('/').pop();
      const mimeType = media.headers['content-type'];
      const mediaDataBase64 = Buffer.from(media.data, 'binary').toString('base64');
      const isVideo = mimeType?.includes('video');
      const mediaMessage = new MessageMedia(mimeType, mediaDataBase64, mediaFilename);
      await clientInstance.sendMessage(number, mediaMessage, {
        sendMediaAsDocument: isVideo,
      });
    }
